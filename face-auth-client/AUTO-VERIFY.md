...
#### 方式2：张嘴检测
- 计算嘴巴高度/宽度比(MAR - Mouth Aspect Ratio)
- 自然张嘴即可触发
- 阈值：MAR > 0.5

### 使用建议
- 检测到人脸后，轻微**左右摇头**或**自然张嘴**
- 任意一种方式通过即可
- 动作自然，不要过于剧烈
- 保持脸部在检测区域内
...
```javascript
{
  QUALITY_THRESHOLD: 70,        // 综合质量阈值（降低后）
  CONSECUTIVE_FRAMES: 2,        // 连续达标帧数（降低后）
  LIVENESS_REQUIRED: true,      // 需要活体检测
  HEAD_TURN_THRESHOLD: 0.15,    // 摇头检测阈值
  MOUTH_OPEN_THRESHOLD: 0.5,    // 张嘴检测阈值
  FACE_SIZE_MIN: 0.12,          // 最小人脸占比
  FACE_SIZE_MAX: 0.65,          // 最大人脸占比
  ANGLE_LIMIT: 25,              // 最大偏转角度
  CONFIDENCE_MIN: 0.6           // 最小检测置信度
}
```
...
- **人脸检测**：face-api.js (TinyFaceDetector + FaceLandmark68Tiny)
- **活体检测**：摇头检测 + 张嘴检测 (MAR算法)
